...
static int CeedOperatorAssembleLinearQFunction_Hip(CeedOperator op) {
  int ierr;
  Ceed ceed;
  ierr = CeedOperatorGetCeed(op, &ceed); CeedChk(ierr);
...
int CeedOperatorCreate_Hip(CeedOperator op) {
  ...
  ierr = CeedSetBackendFunction(ceed, "Operator", op, "AssembleLinearQFunction",
                                reinterpret_cast<int (*)()>(CeedOperatorAssembleLinearQFunction_Hip));
  CeedChk(ierr);
  ierr = CeedSetBackendFunction(ceed, "Operator", op, "ApplyAdd",
                                reinterpret_cast<int (*)()>(CeedOperatorApplyAdd_Hip)); CeedChk(ierr);
  ierr = CeedSetBackendFunction(ceed, "Operator", op, "Destroy",
                                reinterpret_cast<int (*)()>(CeedOperatorDestroy_Hip)); CeedChk(ierr);
  return 0;
}
