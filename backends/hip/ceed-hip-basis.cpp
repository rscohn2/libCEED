...
extern "C" __global__ void interp(const CeedInt nelem, const int transpose,
  ...
  for (CeedInt elem = blockIdx.x*blockDim.z + threadIdx.z; elem < nelem;
    ...
    for(int comp=0; comp<BASIS_NCOMP; comp++) {
      if (!transpose) {//run with QUADS threads
        U = d_U + elem*BASIS_NCOMP*DOFS + comp*DOFS;
        V = 0.0;
        for (int i = 0; i < DOFS; ++i) {
          V += d_B[i+tid*DOFS]*U[i];
        }
        d_V[elem*QUADS + comp*nelem*QUADS + tid] = V;
      } else {//run with DOFS threads
        U = d_U + elem*QUADS + comp*nelem*QUADS;
        V = 0.0;
        for (int i = 0; i < QUADS; ++i) {
          V += d_B[tid+i*DOFS]*U[i];
        }
        d_V[elem*BASIS_NCOMP*DOFS + comp*DOFS + tid] = V;
      }
    }
  }
}
    ...
    for(int comp=0; comp<BASIS_NCOMP; comp++) {
      if (!transpose) {//run with QUADS threads
        ...
        for (int i = 0; i < DOFS; ++i) {
          ...
          for(int dim=0; dim<BASIS_DIM; dim++) {
            V[dim] += d_G[i+tid*DOFS+dim*DOFS*QUADS]*val;
          }
        }
        for(int dim=0; dim<BASIS_DIM; dim++) {
          d_V[elem*QUADS + comp*nelem*QUADS + dim*BASIS_NCOMP*nelem*QUADS + tid] = V[dim];
        }
      } else {//run with DOFS threads
        double V = 0.0;
        for(int dim=0; dim<BASIS_DIM; dim++) {
          U = d_U + elem*QUADS + comp*nelem*QUADS +dim*BASIS_NCOMP*nelem*QUADS;
          for (int i = 0; i < QUADS; ++i) {
            V += d_G[tid+i*DOFS+dim*DOFS*QUADS]*U[i];
          }
        }
        d_V[elem*BASIS_NCOMP*DOFS + comp*DOFS + tid] = V;
      }
...
extern "C" __global__ void weight(const CeedInt nelem,
  ...
  for (CeedInt elem = blockIdx.x*blockDim.z + threadIdx.z; elem < nelem;
       elem += gridDim.x*blockDim.z) {
    d_V[elem*QUADS + tid] = qweight[tid];
  }
}
...
int CeedBasisCreateTensorH1_Hip(CeedInt dim, CeedInt P1d, CeedInt Q1d,
  ...
  ierr = CeedCompileHip(ceed, code.str().c_str(), &data->module, 7,
                         "BASIS_Q1D", Q1d,
                         "BASIS_P1D", P1d,
  ...
  ierr = CeedSetBackendFunction(ceed, "Basis", basis, "Apply",
                                reinterpret_cast<int (*)()>(CeedBasisApply_Hip));
  CeedChk(ierr);
  ierr = CeedSetBackendFunction(ceed, "Basis", basis, "Destroy",
                                reinterpret_cast<int (*)()>(CeedBasisDestroy_Hip));
  CeedChk(ierr);
  return 0;
}

int CeedBasisCreateH1_Hip(CeedElemTopology topo, CeedInt dim,
  ...
  std::ostringstream code;
  code << "\n#include <hip/hip_runtime.h>\n";
  code << kernelsNonTensorRef; 

  ierr = CeedCompileHip(ceed, code.str().c_str(), &data->module, 4,
                         "QUADS", nqpts,
                         "DOFS", nnodes,
                         "BASIS_DIM", dim,
                         "BASIS_NCOMP", ncomp
  ...
  CeedChk(ierr);
  ierr = CeedSetBackendFunction(ceed, "Basis", basis, "Apply",
                                reinterpret_cast<int (*)()>(CeedBasisApplyNonTensor_Hip));
  CeedChk(ierr);
  ierr = CeedSetBackendFunction(ceed, "Basis", basis, "Destroy",
                                reinterpret_cast<int (*)()>(CeedBasisDestroyNonTensor_Hip));
  CeedChk(ierr);
  return 0;
}
