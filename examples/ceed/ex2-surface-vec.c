...
int main(int argc, const char *argv[]) {
  ...
  // representations.
  CeedInt mesh_size, sol_size, full_sol_size;
  ...
  sol_size = full_sol_size/dim;
  if (!test) {
    printf("Number of mesh nodes         : %d\n", mesh_size/dim);
    printf("Number of solution nodes     : %d\n", sol_size);
    printf("Number of vec solution nodes : %d\n", full_sol_size);
  }
  ...
  case 0:
    ...
    CeedQFunctionAddInput(apply_qfunc, "du", dim * dim, CEED_EVAL_GRAD);
    CeedQFunctionAddInput(apply_qfunc, "qdata", dim*(dim+1)/2, CEED_EVAL_NONE);
    CeedQFunctionAddOutput(apply_qfunc, "dv", dim * dim, CEED_EVAL_GRAD);
  ...
  for (CeedInt i = 0; i < sol_size; i++) {
    for (CeedInt comp = 0; comp < dim; comp++) {
      u_host[i + comp * sol_size] = 0.0;
      for (CeedInt d = 0; d < dim; d++)
        u_host[i + comp * sol_size] += x_host[i+d*sol_size];
    }
  }
  ...
  CeedScalar sa_comp = 0.;
  for (CeedInt d = 0; d < dim; d++) {
    for (CeedInt i = d * sol_size; i < (d+1) * sol_size; i++) {
      sa_comp += fabs(v_host[i]);
    }
    printf("\nsa comp: %.14g\n", sa_comp);
    sa_comp = 0.0;
  }
...
int BuildCartesianRestriction(Ceed ceed, int dim, int nxyz[3], int order,
  ...
  printf("scalar_size, ncomp, scalar_size * ncomp: %d, %d, %d\n", scalar_size, ncomp, scalar_size*ncomp);
