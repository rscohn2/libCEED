...
PetscErrorCode BCClamp(PetscInt dim, PetscReal loadIncrement,
  ...
  //const PetscScalar lengthFinal = 25.0;
  u[0] = lx + s*(-kz*y + ky*z) + (1-c)*(-(ky*ky+kz*kz)*x + kx*ky*y + kx*kz*z) + x*(clampMax[7] - 1.0)*loadIncrement;// + lengthFinal*0.05*sin(M_PI*10.0*z/lengthInit)*loadIncrement;
  u[1] = ly + s*(kz*x + -kx*z) + (1-c)*(kx*ky*x + -(kx*kx+kz*kz)*y + ky*kz*z) + y*(clampMax[8] - 1.0)*loadIncrement;
  u[2] = lz + s*(-ky*x + kx*y) + (1-c)*(kx*kz*x + ky*kz*y + -(kx*kx+ky*ky)*z) + z*(clampMax[9] - 1.0)*loadIncrement;// + lengthFinal*0.05*sin(M_PI*10.0*x/lengthInit)*loadIncrement;
  PetscFunctionReturn(0);
};
